...
router.get('/', authMiddleware, (req, res) => {
    ...
    const db = getDb();
    const teamId = req.user?.role === 'admin' ? req.query.team_id : req.user?.team_id;
    if (!teamId) {
...
router.post('/', authMiddleware, (req, res) => {
    ...
    db.run('INSERT INTO team_checklists (team_id, item, category) VALUES (?, ?, ?)', [teamId, item.trim(), category || 'general']);
    saveDb();
