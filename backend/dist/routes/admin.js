...
router.post('/send-reminders', authMiddleware, requireRole('admin'), async (_req, res) => {
    ...
    for (const team of teams) {
        const ok = await sendReminder(team.captain_email, 'Recordatorio: El Campeonato de Sándwiches 2026', `<h2>¡Hola ${team.name}!</h2>
...
router.get('/todos', authMiddleware, requireRole('admin'), (_req, res) => {
    const db = getDb();
...
router.put('/todos', authMiddleware, requireRole('admin'), (req, res) => {
...
router.post('/invites', authMiddleware, requireRole('admin'), (req, res) => {
    ...
    db.run('INSERT INTO invites (code, created_by, role, team_id) VALUES (?, ?, ?, ?)', [code, req.user.email || 'admin', inviteRole, team_id || null]);
    saveDb();
...
router.get('/invites', authMiddleware, requireRole('admin'), (_req, res) => {
    const db = getDb();
