...
async function createTestDb() {
  ...
  for (const file of files) {
    ...
    try { db.run(sql) } catch { /* skip failing migrations */ }
  }
...
function insertChatMessage(overrides: Record<string, any> = {}) {
  const defaults = {
    channel: 'global',
  ...
  db.run(
    ...
    [msg.channel, msg.sender_name, msg.sender_role, msg.content]
  )
}

function insertScore(teamId: number, judgeId: string, category: string, value: number) {
  db.run(
    'INSERT INTO scores (team_id, judge_anonymous_id, category, value) VALUES (?, ?, ?, ?)',
    [teamId, judgeId, category, value]
  )
}
...
describe('chat messages - database operations', () => {
  ...
  it('supports pagination with before parameter', () => {
    ...
    const beforeRows = db.exec(
      "SELECT * FROM chat_messages WHERE channel = 'global' AND id < ? ORDER BY created_at DESC LIMIT 10",
      [lastId]
    )
    const older = rowsToArray(beforeRows)
    expect(older.length).toBeGreaterThan(0)
    expect(older.every((m: any) => m.id < lastId)).toBe(true)
  })
})
...
describe('event_config - database operations', () => {
  it('has default scoring categories', () => {
    ...
    const cats = JSON.parse(config!.scoring_categories as string || '[]')
  ...
  it('can update revealed categories', () => {
    ...
    const config = rowsToObject(rows)
    const revealed = JSON.parse(config!.revealed_categories as string)
