...
async function createTestDb() {
  ...
  for (const file of files) {
    ...
    try { db.run(sql) } catch { /* skip */ }
  }
}

function rowsToArray(rows: any[]) {
...
function insertTeam(overrides: Record<string, any> = {}) {
  ...
  db.run(
    `INSERT INTO teams (name, slug, sandwich_name, captain_email, password_hash, members, status, open_to_join)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    [team.name, team.slug, team.sandwich_name, team.captain_email, team.password_hash, team.members, team.status, team.open_to_join]
  )
}

function insertUser(overrides: Record<string, any> = {}) {
...
function makeToken(role: string, payload: Record<string, any> = {}) {
  return jwt.sign({ role, ...payload }, JWT_SECRET, { expiresIn: '1h' })
}

function request(method: string, urlPath: string, body?: any, headers: Record<string, string> = {}): Promise<{ status: number; body: any; headers: http.IncomingHttpHeaders }> {
...
function createApp() {
  ...
  app.post('/api/auth/admin/login', (req, res) => {
    ...
    if (rows.length === 0 || rows[0].values.length === 0) return res.status(401).json({ error: 'Invalid credentials' })
    const user: Record<string, any> = {}
    rows[0].columns.forEach((c: string, i: number) => (user[c] = rows[0].values[0][i]))
  ...
  app.post('/api/auth/team/login', (req, res) => {
    ...
    rows[0].columns.forEach((c: string, i: number) => (team[c] = rows[0].values[0][i]))
    if (!bcrypt.compareSync(password, team.password_hash)) return res.status(401).json({ error: 'Invalid credentials' })
  ...
  app.post('/api/teams/register', (req, res) => {
    ...
    if (!name || !captain_email || !password) return res.status(400).json({ error: 'Missing required fields' })
    const existing = db.exec('SELECT id FROM teams WHERE name = ?', [name])
    if (existing.length > 0 && existing[0].values.length > 0) return res.status(409).json({ error: 'Team name already taken' })
    const slug = name.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9\s-]/g, '').replace(/[\s_]+/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '')
    const hash = bcrypt.hashSync(password, 10)
    db.run(
      `INSERT INTO teams (name, slug, sandwich_name, captain_email, password_hash, members, open_to_join) VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [name, slug, sandwich_name || '', captain_email, hash, '[]', 0]
    )
    const idRows = db.exec('SELECT last_insert_rowid() as id')
    const id = idRows[0].values[0][0]
    const token = jwt.sign({ team_id: id, team_slug: slug, name, role: 'team' }, JWT_SECRET, { expiresIn: '24h' })
    res.cookie('session_token', token, { httpOnly: true, sameSite: 'strict', path: '/', maxAge: 86400000 })
  ...
  app.post('/api/chat/global/messages', (req, res) => {
    ...
    if (!content || typeof content !== 'string' || content.trim().length === 0) return res.status(400).json({ error: 'Message content is required' })
    ...
    db.run(
      ...
      ['global', user?.id || null, name, role, content.trim()]
    )
    const idRows = db.exec('SELECT last_insert_rowid() as id')
    const id = idRows[0].values[0][0]
  ...
  app.post('/api/chat/team/:teamSlug/messages', (req, res) => {
    ...
    db.run(
      'INSERT INTO chat_messages (channel, sender_id, sender_name, sender_role, content) VALUES (?, ?, ?, ?, ?)',
      [channel, user.team_id || user.id, user.name || 'User', user.role, content.trim()]
    )
    const idRows = db.exec('SELECT last_insert_rowid() as id')
    const id = idRows[0].values[0][0]
    const msgRows = db.exec('SELECT * FROM chat_messages WHERE id = ?', [id])
  ...
  app.post('/api/guests/rsvp', (req, res) => {
    ...
    if (existing.length > 0 && existing[0].values.length > 0) return res.status(409).json({ error: 'Este email ya está registrado' })
  ...
  app.get('/api/guests/validate-access-code', (req, res) => {
    ...
    if (!access_code) return res.json({ valid: false })
    const rows = db.exec('SELECT name FROM guests WHERE access_code = ?', [(access_code as string).trim().toUpperCase()])
    if (rows.length === 0 || rows[0].values.length === 0) return res.json({ valid: false })
    res.json({ valid: true, name: rows[0].values[0][0] as string })
  })
...
beforeEach(() => {
  try { db.run('DELETE FROM chat_messages') } catch {}
  try { db.run('DELETE FROM scores') } catch {}
  try { db.run("DELETE FROM teams WHERE name = 'E2E Team'") } catch {}
  try { db.run("DELETE FROM users WHERE email = 'admin@e2e.com'") } catch {}
  try { db.run("DELETE FROM guests WHERE email LIKE '%@e2e.com'") } catch {}
})
...
describe('E2E: team registration', () => {
  it('registers a new team', async () => {
    ...
    expect(res.body.team.slug).toBe('e2e-team')
...
describe('E2E: guest RSVP', () => {
  ...
  it('validates guest access code', async () => {
    ...
    const code = rows[0].values[0][0] as string
