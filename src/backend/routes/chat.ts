...
router.get('/team/:teamId/messages', validateChannelAccess, (req: AuthRequest, res) => {
...
router.post('/team/:teamId/messages', validateChannelAccess, (req: AuthRequest, res) => {
  const { teamId } = req.params;
  const channel = `team:${teamId}`;
...
router.get('/judge/messages', validateChannelAccess, (req: AuthRequest, res) => {
  const limit = Math.min(parseInt(req.query.limit as string) || 50, 100);
  const before = req.query.before as string | undefined;
...
router.post('/judge/messages', validateChannelAccess, (req: AuthRequest, res) => {
  const { content } = req.body;
...
router.get('/global/messages', (req: AuthRequest, res) => {
  const limit = Math.min(parseInt(req.query.limit as string) || 50, 100);
  const before = req.query.before as string | undefined;
...
router.get('/teams', requireAdmin, (_req: AuthRequest, res) => {
  const teams = queryAll('SELECT id, name FROM teams ORDER BY name');
  res.json({ teams });
});

// DELETE /api/chat/:channel/messages/:messageId (admin only)
router.delete('/:channel/messages/:messageId', requireAdmin, (req: AuthRequest, res) => {
