  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('/api/auth/judge/login', {
        ...
        body: JSON.stringify({ password, anonymous_id: storedUser?.anonymous_id }),
      })
    ...
    <div className="min-h-screen bg-surface flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 max-w-sm w-full space-y-6">
        <div className="text-center">
          <h1 className="font-headline text-3xl font-black text-secondary">Juez</h1>
          <p className="text-gray-500 text-sm mt-1">Acceso para jueces</p>
        </div>
