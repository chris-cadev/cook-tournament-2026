...
export default function TeamChat({ teamId, teamName, onBack }: TeamChatProps) {
  ...
  const loadOlder = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/chat/team/${teamId}/messages?limit=50&before=${oldestId}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      if (res.ok) {
        const data = await res.json()
  ...
  }, [loadingOlder, hasOlder, messages, token, teamId])
      ...
      <main className="flex-1 overflow-y-auto px-4 py-4">
        {loading ? (
          ...
          <div className="max-w-4xl mx-auto flex flex-col gap-3">
            ...
            {messages.map((msg) => {
              const isOwn = msg.sender_id === user?.team_id
              return (
