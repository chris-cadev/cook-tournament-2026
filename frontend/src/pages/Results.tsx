...
export default function Results() {
  ...
  useEffect(() => {
    ...
    socket.on('scores:status', () => {
      fetchLeaderboard()
    })

    return () => {
      socket.off('score:reveal')
      socket.off('scores:status')
    }
  ...
  const { leaderboard, categories, revealed, scores_public } = data
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        {!scores_public ? (
          <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
            <span className="material-symbols-outlined text-5xl text-gray-300 mb-4 block">hourglass_top</span>
            <p className="font-headline text-xl font-bold text-secondary mb-2">
              Las puntuaciones serán reveladas pronto
            </p>
            <p className="text-gray-500">
              Espera a que el administrador active la visualización pública.
            </p>
          </div>
        ) : (
          <>
                    ...
                    {leaderboard.map((entry, idx) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <div className="font-semibold">{allRevealed ? entry.team_name : '???'}</div>
                          <div className="text-sm text-gray-500 sm:hidden">{allRevealed ? entry.sandwich_name : '???'}</div>
                        </td>
                        <td className="px-4 py-3 text-gray-600 hidden sm:table-cell">{allRevealed ? entry.sandwich_name : '???'}</td>
